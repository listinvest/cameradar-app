...
func New(routesFilePath, credentialsFilePath string, fromClient <-chan string, toClient chan<- string) (*Cameradar, error) {
	...
	curler := curl.EasyInit()
	if err != nil || curler == nil {
		return nil, errors.New("libcurl initialization failed")
	}
...
func (c *Cameradar) AttackRoute() ([]cmrdr.Stream, error) {
	streams, err := cmrdr.AttackRoute(c.curler, c.Streams, c.options.Routes, c.options.Timeout, true)
	if err != nil {
		return streams, errors.Wrap(err, "could not discover streams")
	}
	c.Streams = streams
...
func (c *Cameradar) AttackCredentials() ([]cmrdr.Stream, error) {
	streams, err := cmrdr.AttackCredentials(c.curler, c.Streams, c.options.Credentials, c.options.Timeout, true)
	if err != nil {
		return streams, errors.Wrap(err, "could not discover streams")
	}
	c.Streams = streams
